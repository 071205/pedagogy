...
**열린 이슈 3건** — `REV-2026-009`(단답형 태그) · `REV-2026-010`(※확인 사항 상자) ·
`REV-2026-016`(한 단 안 문항 간격). 셋 다 **모양이 틀린 것을 넣으면 없는 것보다 나쁘다**
는 이유로 일부러 손대지 않았다. 앞 둘은 실물 대조가 필요하고, `REV-2026-016` 은
**편집기가 문항 높이를 재서 보내는 설계 결정**이 먼저다(어림으로 빈 문단을 넣으면 하나만
많아도 다음 단으로 밀려 배치가 통째로 어긋난다).
...
- **한글(HWPX) 내보내기 — 베타.** 툴바의 `한글 내보내기` 가 `serve.py` 의 `POST /hwpx` 를
  ...
  - 아직 없는 것: `단답형` 구획 태그(`REV-2026-009`), `※ 확인 사항` 상자(`REV-2026-010`),
    한 단 안 문항 간격(`REV-2026-016` — 실물은 둘째 문항을 단 맨 아래에 붙인다).
