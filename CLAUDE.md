...
▶ **가장 큰 제품 결정이 열려 있다 — 문제를 어떻게 받아들일 것인가**
([`docs/PROBLEM-INTAKE-DESIGN.md`](docs/PROBLEM-INTAKE-DESIGN.md) · 2026-09-19 · **구현 0줄**).
지금은 사진 한 장씩 넣어 문항을 만든다. 사용자는 **PDF 여러 개를 넣고 "이렇게 배치해서
만들어줘"** 를 원한다. ⚠️ **'한 문제에 사진 한 장' 은 계약이 아니라 UI 다** — 조판기도 AI
계약도 여러 문항을 이미 감당한다(막힌 것은 여러 파일·PDF·지시·배치 넷뿐이다).
⚠️ **핵심 충돌**: 묶어 받으면 호출 1회 원가가 **10원 → 수백원**으로 벌어져, 호출 수로 세는
지금 한도 모델(`DailyQuota`·`DAILY_LIMIT`)이 무너진다. 저작권 선도 기능 설계 **전에** 정해야
한다. 해리 ↔ 코덱스 논의 대기이며, 논의 전에 **코드를 건드리지 말 것.**
...
⚠️ **`.claude/` 에 알림 훅 둘과 스킬 하나를 두었다**(2026-09-19). 전부 **막지 않고 알리기만**
하며 언제나 exit 0 이다 — 실제 방어선은 검사이고, 이것들은 그 빨간불을 **편집하는 순간으로
앞당길** 뿐이다. 자기검사는 `check:hooks` 로 `check:fast` 에 걸려 있다(경로가 낡으면 빨간불).
· `pair-guard.mjs` — 지금 여는 파일의 **짝**을 이름으로 댄다(파이썬↔JS 사본, 다섯 경계,
  두 화면의 토큰, Rules 화이트리스트…). 아래 실패 방식 2·3번을 편집 시점에 막으려는 것이다.
· `git-add-guard.mjs` — `git add -A`·`git commit -a` 에 이 폴더의 사정을 알린다(갈래 둘이
  섞여 있을 수 있고 `transcript.txt` 는 커밋하지 않는다). ⚠️ **heredoc 본문은 보지 않는다** —
  커밋 메시지에 적힌 글자에 반응하면 양치기 소년이 된다.
· `skills/hwpx-element` — 새 HWPX 요소를 알아내는 ①~④ 절차. 이미 세 번 밟았다.
· `skills/codex-review` — **Codex 와 왕복하는 절차**(2026-09-21). 어느 경로로 부를지 ·
  첫 질문 골격 · 2회차부터는 새 이견만 · 답을 받고 **근거를 코드로 검증한 뒤** 어디에 적는지.
  ⚠️ 스킬이 가리키는 경로·플래그·마무리 계약이 낡으면 `check:codex-bridge` 가 빨간불을 낸다
  (깨보기 3종으로 확인했다).
⚠️ 이 발상은 외부 플러그인(ECC · MIT)을 **읽고** 가져온 것이고 **코드는 한 줄도 옮기지
...
**빠른 설계 티키타카 계약:** foreground 래퍼에 레일 배정 `--model`·`--effort`를 명시한다.
관리형 상태 추적이 꼭 필요한 보안·데이터 손실·최종 아키텍처 게이트만 공식
`codex:codex-rescue`의 `--effort high`를 쓴다. 첫 질문은 필요한
파일·쟁점·완료 조건과 질문
1~2개만 보내고, 다음 왕복에는 **새로 남은 이견과
직전 답의 필요한 부분만** 보낸다. 같은 문서 전체와 이미 합의한 근거를 다시 넣지 않는다.
**예산의 단위는 사용자 메시지 하나다 — Claude↔Codex 실제 모델 호출 양방향 합산 최대 3회**이고,
다음 메시지에서 다시 3회다(주제가 이어져도 그렇다). 그 안에서는 방향·세션을 합쳐 센다. 시작된
실패·timeout·usage limit 호출도 1회이며, 호출받은 Codex가 Claude를 다시 부르는 중첩 호출은
하지 않는다. 1차 사실 확인 → 2차 새 이견 → 3차 최종 판정으로 끝내고
`합의 / 미합의 / 근거 / 다음 행동`으로 닫는다. 한 에이전트만 구현한 뒤 다른 쪽이 검토한다.

**누가 주도하고 누가 검토하는가 — 단계별 계약**(2026-09-22 · 코덱스와 합의 · `HANDOFF-2026-160`)

| 단계 | 기본 | **Codex 선행으로 바꾸는 조건** |
|---|---|---|
| 설계 | Claude 가 의도·범위·완료 조건을 정리 | 인증·권한 · 사용자 데이터 손실 · 저장/병합 · 동시성·멱등성 · 스키마·프로토콜 · 되돌리기 어려운 변경 · **근거가 갈리는 주장** 중 하나라도 있으면 **Codex 가 1차에 불변조건과 실패 모델을 먼저 판정** |
| 구현 | **한 명만.** 기본은 Claude | 아래 세 조건을 **모두** 만족하면 Codex |
| 검토 | **반드시 구현하지 않은 쪽** | Claude 구현 → Codex · **Codex 구현 → Claude** |
| 최종 게이트 | Claude 가 검사·기록·미결 위험을 종합 | 방향 자체를 아직 안 공격했으면 `/codex:adversarial-review` 를 **사용자에게 권한다** |

⚠️ **Codex 구현을 고르는 기준은 "백엔드라서" 가 아니다** — 셋을 모두 만족할 때다.
① 핵심 난점이 UI 의도가 아니라 **상태 전이와 불변조건**이다 ② 올바름을 테스트·에뮬레이터·
깨보기로 **기계적으로 닫을 수 있다** ③ 그래도 **Claude 가 독립 검토할 수 있다**.
반대로 UX 의도·인쇄 외형·사용자 문구·여러 화면의 맥락 조율은 **Claude 구현이 기본**이다.
⚠️ **연속 레일의 구현자를 중간에 바꾸지 않는다**(지금 R4/B1~B3 저장 레일은 Codex 다).

⚠️ **`/codex:review`·`/codex:adversarial-review` 는 Claude 가 부를 수 없다**
(`disable-model-invocation: true` — 실제로 확인했다). **사용자가 직접 쳐야 한다.**
그래서 **기본 경로가 아니라 보조 최종 게이트**이고, 아래 조건일 때만 권한다.
· `/codex:review --wait` — 래퍼 검토가 **실패했는데 사용량 제한이 원인이 아닐 때**, 또는
  배포 직전 branch diff 를 내장 검토기로 한 번 볼 이유가 있을 때.
  ⚠️ **같은 범위를 래퍼가 이미 검토했으면 권하지 않는다**(중복).
· `/codex:adversarial-review --wait <초점>` — 보안·인증·사용자 데이터·저장/병합·동시성·
  최종 아키텍처 중 하나를 건드리고 · 검토 가능한 diff 가 있고 · 기존 검토가 **결함만 보고
  "이 방향이 맞는가" 는 안 봤을 때**. 셋을 **모두** 만족할 때만.
⚠️ **`--wait` 를 반드시 붙인다** — 이 저장소는 background 금지다.
⚠️ **사용자가 친 slash 명령도 실제 모델 호출이므로 그 턴의 3회에서 1회다.**

⚠️ **공식 Stop review gate(`/codex:setup --enable-review-gate`)를 켜지 말 것.**
그 훅은 `decision:"block"` 으로 **종료를 실제로 막는다**(소스에서 확인했다 ·
`stop-review-gate-hook.mjs:169`). 이 저장소의 훅은 **막지 않고 알리기만** 한다.
기본값은 꺼짐이므로 지금 할 일은 없다 — **켜지 않는 것**이 결정이다.
⚠️ **Codex 를 여러 개 병렬로 돌리지 않는다** — 밖에는 4개 fan-out 하는 구현이 있지만
합산 3회 예산과 정면으로 어긋난다.
...
`codex-final.md`와 진행 로그의 `model:`·`reasoning effort:` 실행 메타데이터만 확인한다.
나머지 진행 로그는 실패했을 때만 읽는다. 위 예시는 일반 독립 검토 배정이며 설계 판정은
레일대로 `--model gpt-6-astra --effort high` 등으로 바꾼다. 질문은 한 파일에 쟁점과 완료 조건을
묶고 위 한 턴 공용 3회 예산을 지킨다. 한 번에 한 에이전트만 코드를 쓰며 구현 뒤에는
...
`node scripts/hwp-to-pdf.mjs 파일.hwpx` 가 '파일 → PDF로 저장하기' 를 눌러 준다
(단축키 ⌘S·⌘P 는 이 앱에 먹지 않는다 — 메뉴를 눌러야 한다. 저장 대화상자는 시트가 아니라
별도 창이고 '저장' 단추는 AX 이름이 없어 **가장 오른쪽 단추**로 찾는다).
⚠️ 머리말을 `secPr` 에 넣으면 **열리는데 아무것도 안 찍히던** 것도 이 단계에서만 드러났다.
...
- ⚠️ **한글이 자고 있으면 첫 판정이 거짓 실패로 나온다.** 창이 뜨기 전에 검사가 포기한다 —
  한 건만 ❌ 이고 뒤 파일은 ✅ 라면 파일이 아니라 **콜드 스타트**를 의심하고 다시 돌린다
  (2026-09-17 에 실제로 겪었다. 그 파일은 다시 돌리니 열렸다).
...
- **계약이 제품이다.** `document_schema.py` 가 받는 블록만 조판된다. 지금 **13종**:
  ...
  ⚠️ **머리말·꼬리말은 블록이 아니라 문서 수준 값이다**(2026-09-18) — 최상위
  `header`·`footer`(각 120자). 글의 흐름에 끼는 것이 아니라 구역 전체에 걸리기 때문이다.
  ⚠️ **`secPr` 안에 넣고 `<hp:headerApply>` 로 가리키지 말 것 — 한글이 파일은 열어 주고
  아무것도 안 찍는다.** 본문 첫 문단의 `secPr` 를 안고 있는 run 에 `<hp:ctrl>` 로 매단다.
  변종 둘을 PDF 로 뽑아 갈랐다(근거·XML 은 `docs/HWPX-ELEMENT-SPECS.md`).
  아직 없는 것: 상자 안의 표·그림 · 지문(passage) —
  근거를 만드는 절차는 [`docs/HWPX-ELEMENT-SPECS.md`](docs/HWPX-ELEMENT-SPECS.md) 에 있다.
⚠️ **브라우저도 내보내기 직전에 검증한다**(2026-09-17 · `HANDOFF-2026-143`).
`toBlob()` 이 `strictValidate()` 를 부른다 — 예전에는 파이썬 `save()` 만 검사하고 **정작
사용자에게 나가는 브라우저 경로는 아무것도 보지 않았다.** 보는 것은 필수 파트 · 참조표의
`itemCnt` · 끊어진 id 참조 · 글꼴 참조다. 전부 **XML 문법은 완벽한데 한글만 거부하는**
종류라 파서로는 안 잡힌다.
⚠️ **그래서 `toBlob()` 은 던질 수 있다.** 정상 문서를 거부하면 내보내기가 통째로 막히므로,
참조표가 **통째로 없는** 경우는 오류가 아니라 **경고**로 둔다(그런 문서를 한글이 여는 것을
확인했다). 이 구분을 지우지 말 것.
⚠️ 검증기도 사본이라 갈라진다 — `test:hwpx-browser` 가 **고장 다섯을 심어** 양쪽이 잡는지와
**문구까지 같은지**를 본다.

⚠️ **문단 모양을 바꿀 때 직계 run 만 건드릴 것**(`REV-2026-096`). `set_paragraph_style()`
이 문단을 깊이 훑던 동안, 제목을 첫 문단에 이어 쓰는 순간 그 문단에 매달린 **머리말 속
run 까지 제목 모양(19pt)으로** 바뀌었다 — 머리말·꼬리말·각주는 **제 스타일을 가진 별도 글**이다.
각주도 같은 자리에 있었다(아직 안 터졌을 뿐이다). 파이썬·JS 양쪽을 고쳤다.

⚠️ **각주 본문 스타일은 이름으로 찾는다**(`REV-2026-094`). `blank.hwpx` 는 각주=14·미주=15
인데 코드가 15(미주)를 박고 있었다. 둘이 같은 `paraPr`·`charPr` 을 가리켜 **PDF 로도 안
보였다.** 숫자를 되돌리지 말 것 — 근거는 `docs/HWPX-ELEMENT-SPECS.md`.
...
⚠️ **새 블록 하나는 다섯 곳을 함께 고쳐야 실제로 쓸 수 있다.**
`document_schema.py`(계약) · Worker 프롬프트 · Worker 검증(`validateDocumentResponse`) ·
`document-editor.html` 의 **`validate()`** · 같은 파일의 **`render()`**. 한 곳만 늘리면
**AI 가 정확히 만들어도 Worker 가 502 로 버리고 브라우저에서도 막힌다**
(`REV-2026-013` 에서 실제로 그랬다).
`npm run check:static` 안의 `check-document-blocks.mjs` 가 다섯 곳을 대조한다.
⚠️ **`validate()` 와 `render()` 를 한 칸으로 세지 말 것**(`REV-2026-095`). 검사가
`validate()` 만 보던 동안 `pagebreak`·`footnote` 가 **미리보기에서만 빠진 채로 초록불**이었고,
넣어 보면 `render()` 의 마지막 `else` 가 목록으로 보고 `b.items` 를 훑어 **미리보기가 통째로
죽었다.** 통과하는 것과 그려지는 것은 다른 경계다.
...
⚠️ **외부 라이브러리(`python-hwpx`)는 근거 생성기이지 코드를 가져오는 곳이 아니다** —
파이썬 55,000줄은 브라우저로 못 가고 **파이썬↔JS 대조를 죽인다.** 규모·라이선스·
가져올 값이 있는 API 목록은 그 문서에 정리해 두었다.
