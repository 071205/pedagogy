...
들어와 여덟 가지를 고쳤다(테두리·각주·선지 배치·문단 들여쓰기·순서 문항 (A)(B)(C) 라벨·
듣기 답란·지문 없는 묶음 안내·**안내문 상자**). **② 어휘 세로 2단 네모만 남았고 그것은
실물 근거가 없어 못 한다.**
...
`06-english-notice` 가 안내문 상자를, `07-math-image-choices` 가 그림 선지(3+2 ·
라벨은 그림 위)를 지킨다. 지우지 말 것.
...
- **선지의 그림 (`choices.data.images`)**: 교과서의 그래프 이동 문항처럼 **선지 자체가
  ...
  ⚠️ **그림을 가진 블록이 늘면 `blockImageUrls()` 한 곳만 고친다** — 정리 코드가 `image`
  블록만 알던 시절에는 선지 그림을 지워도 Storage 원본이 영영 남았다.
  ⚠️ **그리고 `storagePathsOf()`(문제집 단위)가 반드시 그것을 거치게 둔다.** 처음엔 안
  거쳤고, 그래서 `releaseImage()` 가 **다른 문제집이 아직 쓰는 파일을 지웠다**(`REV-2026-019`).
  두 수집기가 서로 다른 것을 알면 블록 단위 검사가 전부 초록불인 채로 지나간다 —
  회귀가 이제 **둘의 관계**(블록 단위 ⊆ 문제집 단위)를 본다.
