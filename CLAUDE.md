...
**호출 횟수 제한은 없다(2026-09-24 해리 결정 — 예전의 '한 턴 합산 3회'는 폐지).** 필요한 만큼
왕복하되 **비용은 여전히 든다**: 매 왕복은 새로 남은 이견과 필요한 근거만 보내고, 합의한 것을 다시
논하지 않는다. 호출받은 Codex가 Claude를 다시 부르는 중첩 호출은 하지 않는다. 각 왕복은
...
⚠️ 사용자가 친 slash 명령도 실제 모델 호출이다(횟수 제한은 없지만 비용은 든다).
...
여러 Codex가 같은 판단을 따로 내리면 합치는 비용이 더 크다.
...
묶는다(호출 횟수 제한 없음 · 2026-09-24). 한 번에 한 에이전트만 코드를 쓰며 구현 뒤에는
