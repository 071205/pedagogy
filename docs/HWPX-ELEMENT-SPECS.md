...
## ⚠️ 외부 라이브러리는 **근거 생성기**다 — 코드를 가져오는 곳이 아니다

위 ①에서 `python-hwpx` 를 부르는 것은 **그 요소가 든 실물 `.hwpx` 를 얻기 위해서**다.
소스를 베껴 오는 것이 아니다. 경계를 분명히 해 둔다(2026-09-17 에 실제로 전수 분석했다).

| | |
|---|---|
| 규모 | `python-hwpx` 55,078줄 ↔ 우리 2,777줄(파이썬+JS) |
| 라이선스 | Apache-2.0 — 베껴도 되지만 `LICENSE`·`NOTICE` 동봉과 변경 고지 의무가 따라온다 |
| **결정적 이유** | **우리 제품은 JS 다.** 파이썬 55,000줄은 브라우저로 갈 수 없고, 옮겨 오면 **파이썬↔JS 대조가 영구히 죽는다**(`test:hwpx-browser`). 이 저장소가 세 번 겪은 "베낀 것은 갈라진다" 그대로다 |
| 그쪽 공개 API | `add_*` 49종. 그중 우리에게 값이 있는 것은 **머리말·꼬리말 · 미주 · 하이퍼링크 · 도형** 정도다. 변경추적·메모·양식필드·차트·목차·색인·덧말·드롭캡은 문제집/시험지 조판기에 쓸 데가 없다 |

⚠️ **그래도 읽을 값은 있다.** 2026-09-17 에 읽어서 결함 둘을 찾았다 —
...
⚠️ **각주 본문 문단의 `styleIDRef` 를 숫자로 박지 말 것 — 이름으로 찾는다**(`REV-2026-094`).
박아 뒀다가 실제로 틀렸다: 우리 `blank.hwpx` 는 **각주=14 · 미주=15** 인데 코드가 15를
쓰고 있었다. 두 스타일이 같은 `paraPr=10`·`charPr=3` 을 가리켜 **PDF 눈검사로도 안 보였다.**
시험지 틀에서 15 는 `머리쪽번호` 이고, 각주 스타일이 **없는** 문서라면 끊어진 참조가 되어
한글이 문서를 통째로 열지 못한다. `_note_style_refs()` / `_noteStyleRefs()` 가 머리글
스타일 표에서 `각주`·`Footnote` 를 찾고, **못 찾으면 첫 스타일로 떨어진다**(모양이 조금
다른 것이 못 여는 것보다 낫다).
