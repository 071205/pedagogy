...
4. **분리**: production과 staging의 Firebase 프로젝트, Cloudflare Worker, AI 공급자 key, 결제 키를
...
9. **제품 약속**: 한국어 개인용 웹 핵심과 로컬 Typst 확장 기능의 지원 범위를 확정한다.
   서버/글꼴 없는 환경의 출력 경로를 검증하고 미구현 협업·좌석·다국어를 판매하지 않는다.
   서버 조판을 제공한다면 폰트·템플릿 권리와 격리/비용 설계를 별도로 검증한다.
10. **저장 안전성**: 두 기기 동시/오프라인 수정의 충돌 사본 보존, 문서 크기 사전 방어,
    배치 일부 실패 안내와 export/복구를 검증한다. 자동 LWW 덮어쓰기와 크기 실패 안내만으로 완료하지 않는다.
11. **관측과 출력 품질**: 개인정보 최소화 정책 아래 저장·AI·내보내기 성공/실패를 파악할 수 있어야 한다.
    실제 Safari/iOS와 한글 출력, CI에서 제외한 비교 범위의 증거를 남긴다. 호출 로그는 제품 퍼널이 아니다.
12. **공급자 채택**: Gemini를 선택한다면 실제 응답·usage/생각 토큰 과금·오류·개인정보 계약을 검증하고
    런북·보안 체크리스트·법무의 Anthropic 전용 기술을 선택한 공급자에 맞춘다. 후보 상태에서는
    기존 운영 문서를 이미 전환된 것처럼 바꾸지 않는다. 2회 smoke는 품질/비용 우위 증명이 아니다.
