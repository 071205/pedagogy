...
작성: 2026-09-13, 최종 갱신: 2026-09-20. 최초 읽기 전용 조사와 이후 staging 증거를 구분한다.
...
### 2026-09-19~20 OPS-7 최신 결론과 마감 상태

| 항목 | 확인 결과 |
| --- | --- |
| 모델·코드 | 신규 프로젝트에서 404가 난 2.5 대신 `gemini-3.1-flash-lite`를 사용한다. `thinkingLevel="low"`와 표준 오류 코드 allowlist를 반영했고 Worker 검사를 통과했다. |
| 확정 원인 | 승인된 문서 호출에서 `http_status=400`, `provider_error_status=FAILED_PRECONDITION`을 얻었다. Google Cloud 콘솔도 staging 프로젝트에 결제 계정이 없다고 표시했다. 현재 502의 원인은 결제 전제 미충족이다. |
| staging Worker | 활성 version `8a4f3aa4-b70d-452e-bf76-ca5d8b29bd3d`, `DAILY_LIMIT=2`, `GLOBAL_DAILY_LIMIT=2`, logs 10%, Gemini 3.1 Flash-Lite다. production은 변경하지 않았다. |
| 인증 정리 | 2026-09-20 Firebase `/u/1` 콘솔에서 익명 로그인 제공업체가 `사용 중지됨`임을 새로고침 후 확인했다. 현재 임시 인증 파일은 없다. 사용자 목록에는 2026-09-14 생성 익명 테스트 사용자 1개가 남아 있으며 영구 삭제는 별도 확인이 필요하다. |
| 다음 조건 | 사용자가 staging 결제 계정 연결 여부를 결정한다. 연결할 경우 새 실호출 승인 후 `wrangler tail`을 붙여 문서 1회만 검증한다. 연결하지 않으면 Gemini 접근 불가로 OPS-7/10 판단을 마감한다. |

이 최신 표가 위 2026-09-14 당시의 진단·복원 기록보다 우선한다. 결제 연결 전 추가 호출은 같은
`FAILED_PRECONDITION`을 반복할 뿐이므로 실행하지 않는다.
