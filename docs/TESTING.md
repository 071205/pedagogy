...
기준 문제집을 실제 전체 가져오기 input으로 불러온 뒤, 인쇄용 A4 첫 페이지 PNG를
승인된 기준본과 비교한다. 실패하면 CI가 실제 결과와 차이 이미지를 artifact로 남긴다.

⚠️ **기준본은 만든 운영체제별로 있다** — `test-fixtures/visual-baseline/<platform>/`.
글리프 래스터화가 OS 마다 달라 다른 OS 의 기준본과는 견줄 수 없다(맥 기준본 ↔ CI 리눅스가
쪽의 1.843% 인데, 본문이 3px 밀린 진짜 회귀는 1.492% 다 — 잡음이 신호보다 크다).
그 OS 의 기준본이 없으면 **건너뛴다**(`⏭`). 건너뜀은 통과가 아니다.

⚠️ **판정은 픽셀 비율이 아니라 '행 먹 분포' 다.** 예전에는 전체 픽셀의 0.5% 를 허용치로
썼는데, 쪽 대부분이 흰 여백이라 **글줄 하나가 통째로 사라져도 0.038%** 라서 그냥 통과했다
(`REV-2026-018`). 지금은 픽셀 0.05% 와 행 먹 분포 0.1% 를 함께 본다. 매 실행 끝에 기준본의
글줄을 지워 '지금 설정으로 잡히는가' 를 자기검사한다.
